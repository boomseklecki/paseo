 ...
 * translator — so `@getpaseo/protocol/rules` stays a pure function and
...
export interface RuleMeasurementInput {
  head: readonly StreamItem[];
  tail: readonly StreamItem[];
  lastUserMessageAt: Date | null;
...
export function buildRuleMeasurementContext(input: RuleMeasurementInput): RuleMeasurementContext {
 ...
 */
const MESSAGE_KEY_BY_MEASUREMENT: Record<RuleTrigger, string> = {
  message: "composer.rules.generic",
  always: "composer.rules.generic",
  ...
  "agent.provider": "composer.rules.generic",
  "agent.model": "composer.rules.generic",
  "agent.lastError": "composer.rules.lastError",
  "agent.secondsSinceUserMessage": "composer.rules.secondsSinceUserMessage",
  "agent.contextRemainingTokens": "composer.rules.contextRemainingTokens",
  "agent.idleSeconds": "composer.rules.idleSeconds",
  "agent.contextUsedPercent": "composer.rules.contextUsedPercent",
  "agent.sessionCostUsd": "composer.rules.sessionCostUsd",
};
 ...
 */
import { formatRuleTriggerValue } from "@getpaseo/protocol/rules/format";

export { formatRuleTriggerValue as formatTriggerValue };
 ...
 */
export function defaultRuleWording(
  trigger: string,
  operand: number | string | undefined,
  t: RuleTranslate,
): string {
  const rendered = formatRuleTriggerValue(trigger, operand);
  return t(MESSAGE_KEY_BY_NAME[trigger] ?? "composer.rules.generic", {
    value: rendered,
    threshold: rendered,
...
export function formatRuleFinding(finding: RuleFinding, t: RuleTranslate): string {
  const values = {
    value: formatRuleTriggerValue(finding.trigger, finding.value),
    threshold: formatRuleTriggerValue(finding.trigger, finding.operand),
  ...
  const sentence = finding.message
    ...
    : t(MESSAGE_KEY_BY_NAME[finding.trigger] ?? "composer.rules.generic", values);

  if (finding.disposition !== "block") {
    return sentence;
  }
  return `${sentence} ${t("composer.rules.overrideHint")}`;
}

/** Every finding as one string, most severe first so a block leads. */
export function formatRuleFindings(findings: readonly RuleFinding[], t: RuleTranslate): string {
  const blocking = findings.filter((finding) => finding.disposition === "block");
  const ordered = blocking.length > 0 ? blocking : findings;
  return ordered.map((finding) => formatRuleFinding(finding, t)).join(" ");
}
...
export const RULE_OVERRIDE_WINDOW_MS = 60_000;

export function isRuleOverrideValid(
  ...
  return (
    ...
    candidate.nowMs - override.atMs <= RULE_OVERRIDE_WINDOW_MS &&
