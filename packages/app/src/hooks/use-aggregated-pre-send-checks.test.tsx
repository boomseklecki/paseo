...
import { useAggregatedPreSendChecks } from "./use-aggregated-pre-send-checks";
...
interface FakeHost {
  ...
  supported: boolean;
  list: () => Promise<{ checks: PreSendCheckRule[] }>;
}
  ...
  getHostRuntimeStore: () => ({
    ...
    getSnapshot: (serverId: string) => {
      ...
      return {
        ...
        client: { preSendChecksList: host.list },
      };
    },
  }),
}));
...
describe("useAggregatedPreSendChecks", () => {
  ...
  it("reports a failed list rather than waiting on it forever", async () => {
    ...
    const { result } = renderHook(() => useAggregatedPreSendChecks(), { wrapper });
