...
import { groupRules, type RuleGroup, type RuleHostRules } from "@/screens/settings/rule-groups";

export interface RuleHostState extends RuleHostRules {
  isConnected: boolean;
  isSupported: boolean;
...
export interface AggregatedRules {
 ...
 */
export function useAggregatedRules(): AggregatedRules {
  const { t } = useTranslation();
  const hosts = useHosts();
  const serverIds = useMemo(() => hosts.map((host) => host.serverId), [hosts]);
  const statuses = useHostRuntimeConnectionStatuses(serverIds);
  const features = useHostFeatureMap(serverIds, "rules");

  const queries = useReplicaQueries<readonly Rule[]>(
    hosts.map((host) => ({
      queryKey: rulesQueryKey(host.serverId),
  ...
  const hostStates: RuleHostState[] = hosts.map((host, index) => {
