...
import type { RuleOutcomeDescriptor, RuleExample } from "@getpaseo/protocol/rules/types";
import { useFetchQuery } from "@/data/query";
import { RULES_QUERY_PREFIX } from "@/data/rules";
import { getHostRuntimeStore } from "@/runtime/host-runtime";

export interface RuleCatalog {
  /** What the daemon can carry out, so the editor can offer it untaught. */
  outcomes: RuleOutcomeDescriptor[];
  /** Rules the daemon suggests, none of them installed. */
  examples: RuleExample[];
}

const EMPTY: RuleCatalog = { outcomes: [], examples: [] };
 ...
 */
export function useRuleCatalog(serverId: string | null): RuleCatalog {
  const { t } = useTranslation();

  const query = useFetchQuery({
    queryKey: [RULES_QUERY_PREFIX, "catalog", serverId],
