...
/**
 * Holds a host's rules for the composer to consult at send time.
 *
...
export function useRules(serverId: string | null): UseRulesResult {
  ...
  const supported = useHostFeature(serverId, "rules");
  const queryKey = useMemo(() => rulesQueryKey(serverId), [serverId]);
  ...
  const checksQuery = useReplicaQuery({
    ...
    queryFn: async () => {
      ...
      const result = await client.rulesList();
      return result.checks;
    },
  });

  const readRules = useCallback((): readonly Rule[] | null => {
    ...
    return queryClient.getQueryData<readonly Rule[]>(queryKey) ?? null;
  }, [queryKey, supported]);
 ...
 * have them.
 */
export function useRulesHostGap(serverId: string | null): boolean {
  const normalizedServerId = serverId?.trim() ?? "";
  const isConnected = useHostRuntimeIsConnected(normalizedServerId);
  const supportedHere = useHostFeature(normalizedServerId, "rules");
  const supportedElsewhere = useSessionStore((state) =>
    Object.entries(state.sessions).some(
      ([otherServerId, session]) =>
        otherServerId !== normalizedServerId && hostSupportsFeature(session?.serverInfo, "rules"),
    ),
  );
