...
export const ru: TranslationResources = {
  ...
  settings: {
    ...
    preSendChecks: {
      ...
      hostsLabel: "Хосты",
      hostsHint: "На каких машинах действует это правило. Оно хранится на каждой отдельно.",
      hostToggle: "Сохранить это правило на {{host}}",
      hostsRequired: "Выберите хотя бы один хост.",
      hostsSucceeded: "Сохранено на {{hosts}}.",
      hostFailed: "{{host}}: {{reason}}",
      hostUnreachable: "Нет подключения.",
      featureToggleHost: "Выполнять правила на {{host}}",
      tryItLabel: "Проверить",
      tryItPlaceholder: "Введите, на чём проверить",
      verdicts: {
        fires: "Это правило сработает.",
        "condition-false": "На этом правило не сработает.",
        disabled: "Это правило выключено.",
        "unknown-event": "Эта сборка не знает такого момента.",
        "trigger-not-at-this-event": "Этот триггер ничего не значит в этот момент.",
        "outcome-not-at-this-event": "Этот исход невозможен в этот момент.",
      },
      neverFires: "Никогда не сработает",
      differs: "Расходится",
      ...
      featureToggleTitle: "Выполнять правила",
      ...
      examples: {
        "aside-on-btw": {
          ...
          description:
            "Сообщение, начинающееся с /btw, не отправляется: на него отвечает скрытый агент, а ответ появляется среди субагентов.",
        },
        "notify-on-failed-turn": {
          label: "Сообщать о неудачном ходе в дорогой сессии",
          description:
            "Только после того, как сессия превысит сумму, ради которой стоит вас отвлекать.",
        },
        "notify-agent-left-idle": {
          label: "Сообщать об агенте, оставленном без дела",
          description: "Агент, ждущий вас, выглядит так же, как завершённый.",
        },
        "notify-context-pressure": {
          label: "Сообщать, что разговор почти заполнен",
          description:
        ...
        "fork-on-slash-fork": {
          ...
          description:
            "Кнопка ответвления как выбранный вами ярлык: /fork переносит разговор в новый вместо отправки.",
        },
        "start-fresh-when-full": {
          label: "Открывать новый разговор, когда этот заполнится",
          description:
            "Рядом открывается новый разговор, ничего не перенося — используйте вместе с правилом передачи.",
        },
        "retry-after-a-failed-turn": {
          label: "Повторить через десять минут после неудачного хода",
          description:
            "Чаще всего это лимит запросов или сбой — помогает подождать, а не разбираться.",
        },
        "warn-context-nearly-full": {
          label: "Предупреждать, когда контекст почти заполнен",
          description: "На 80 %, пока закончить мысль всё ещё дешевле, чем начинать заново.",
        },
        "warn-session-cost": {
          label: "Предупреждать при превышении стоимости сессии",
          description: "Предупреждение, а не блокировка: значимая сумма меняется день ото дня.",
        },
        "block-cold-prompt-cache": {
          label: "Блокировать, когда кэш промпта остыл",
          description: "После часа следующий ход заново обрабатывает весь разговор.",
        },
      },
      ...
      remove: "Удалить",
      removeConfirmTitle: "Удалить это правило?",
      ...
      messageTokenTyped: "{{message}} заменяется тем, что вы напечатали.",
      messageTokenFromRule:
        "{{message}} заменяется полем «Сообщение» ниже — в этот момент никто ничего не печатает. Оставляйте его пустым, только если промпт не использует токен.",
      messageNeededForToken:
        "Этот промпт использует {{message}}, а в этот момент никто ничего не печатает. Напишите сообщение или уберите {{message}} из промпта.",
      messageHint:
        "Необязательно и используется всеми итогами выше — текст предупреждения, тело уведомления и {{message}}, который получает промпт.",
