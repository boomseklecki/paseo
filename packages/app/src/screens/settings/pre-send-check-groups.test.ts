...
function rule(overrides: Partial<PreSendCheckRule> & { id: string }): PreSendCheckRule {
  return { ...RULE, ...overrides };
}

function host(serverId: string, rules: readonly PreSendCheckRule[] | null): PreSendCheckHostRules {
...
describe("groupPreSendCheckRules", () => {
  it("collects one rule carried by two hosts into a single group", () => {
    const groups = groupPreSendCheckRules([host("a", [RULE]), host("b", [RULE])]);
  ...
  it("keeps the first host's arrangement and appends what only later hosts have", () => {
    const groups = groupPreSendCheckRules([
  ...
  it("ignores a host whose rules are not loaded", () => {
    const groups = groupPreSendCheckRules([host("a", [RULE]), host("b", null)]);
  ...
  it("does not count a differing key order as disagreement", () => {
    const reversed = Object.fromEntries(
      Object.entries(RULE).toReversed(),
    ) as unknown as PreSendCheckRule;
    expect(groupPreSendCheckRules([host("a", [RULE]), host("b", [reversed])])[0]?.differs).toBe(
      false,
    );
  });
  ...
  it("counts an unknown field a newer daemon added as disagreement", () => {
    const groups = groupPreSendCheckRules([
      host("a", [RULE]),
      host("b", [{ ...RULE, severity: "high" } as PreSendCheckRule]),
    ]);
    expect(groups[0]?.differs).toBe(true);
  });
});

describe("planPreSendCheckSave", () => {
  it("writes to every target, not only the newly selected ones", () => {
    expect(planPreSendCheckSave({ currentServerIds: ["a"], targetServerIds: ["a", "b"] })).toEqual({
      write: ["a", "b"],
      remove: [],
    });
  });
  ...
  it("removes the rule from a deselected host", () => {
    expect(planPreSendCheckSave({ currentServerIds: ["a", "b"], targetServerIds: ["b"] })).toEqual({
      write: ["b"],
      remove: ["a"],
    });
  });
  ...
  it("has nothing to write for a rule taken off every host", () => {
    expect(planPreSendCheckSave({ currentServerIds: ["a"], targetServerIds: [] })).toEqual({
      write: [],
      remove: ["a"],
    });
  });
...
describe("describePreSendCheckOutcomes", () => {
  it("has nothing to report when every host succeeded", () => {
    expect(describePreSendCheckOutcomes([{ serverId: "a", serverName: "A", error: null }], t)).toBe(
      null,
    );
  });
  ...
  it("names the hosts that succeeded alongside the one that failed", () => {
    const message = describePreSendCheckOutcomes(
      [
  ...
  it("reports a total failure without claiming a success", () => {
    ...
    expect(message).toBe("settings.preSendChecks.hostFailed(B|offline)");
  });
});
