...
import type { PreSendTranslate } from "@/composer/pre-send-checks";
...
export interface PreSendCheckHostRules {
  serverId: string;
  serverName: string;
  /**
...
export interface PreSendCheckGroup {
  id: string;
  /**
 ...
 * nothing.
 */
export function groupPreSendCheckRules(
  hosts: readonly PreSendCheckHostRules[],
): PreSendCheckGroup[] {
  const groups = new Map<string, PreSendCheckGroup>();
 ...
 */
export interface PreSendCheckSavePlan {
  write: string[];
  remove: string[];
}

export function planPreSendCheckSave(input: {
  currentServerIds: readonly string[];
  targetServerIds: readonly string[];
}): PreSendCheckSavePlan {
...
export interface PreSendCheckHostOutcome {
  serverId: string;
  serverName: string;
  error: string | null;
}
 ...
 */
export function describePreSendCheckOutcomes(
  ...
  const parts = failed.map((outcome) =>
    t("settings.preSendChecks.hostFailed", {
  ...
  if (succeeded.length > 0) {
    parts.unshift(
      t("settings.preSendChecks.hostsSucceeded", {
 ...
 */
function fingerprintRule(rule: PreSendCheckRule): string {
