...
import {
  ...
  isTextTrigger,
  ruleTokensForPrompt,
  ruleTokensForWording,
  type RuleOutcomeDescriptor,
  type RuleOutcomeParameter,
} from "@getpaseo/protocol/rules/types";
import { settingsStyles } from "@/styles/settings";
import { dryRunRuleSample } from "@getpaseo/protocol/rules/dry-run";
import {
  gateRuleSave,
  ruleChoosesHosts,
  addRuleOutcome,
  ...
  type RuleOutcomeDraft,
} from "./rule-form";

export interface RuleModalHost {
  serverId: string;
  serverName: string;
...
interface RuleEditModalProps {
  ...
  onClose: () => void;
  onSave: (draft: RuleDraft, serverIds: readonly string[]) => Promise<void>;
  testID?: string;
}
...
function tokenHelp(tokens: readonly string[], t: TFunction): string {
  const names: Record<string, string> = {
    "{{message}}": t("settings.rules.tokenMessage"),
    "{{value}}": t("settings.rules.tokenValue"),
    "{{threshold}}": t("settings.rules.tokenThreshold"),
    "{{duration}}": t("settings.rules.tokenDuration"),
  };
  const lines = tokens.map((token) => `${token} ${names[token] ?? ""}`.trim());
  return `${t("settings.rules.tokensLabel")} ${lines.join("   ")}`;
}
...
const OPTION_LABEL_PREFIX: Record<PickerKind, string> = {
  event: "settings.rules.events.",
  trigger: "settings.rules.triggers.",
  operator: "settings.rules.operators.",
};

const THRESHOLD_UNIT_KEYS: Record<string, string> = {
  "agent.idleSeconds": "settings.rules.thresholdUnits.idleSeconds",
  "agent.secondsSinceUserMessage": "settings.rules.thresholdUnits.idleSeconds",
  "agent.contextRemainingTokens": "settings.rules.thresholdUnits.contextRemainingTokens",
  "agent.contextUsedPercent": "settings.rules.thresholdUnits.contextUsedPercent",
  "agent.sessionCostUsd": "settings.rules.thresholdUnits.sessionCostUsd",
};
...
function RulePicker({
  ...
  const labelFor = useCallback(
    (option: string) =>
      known.includes(option)
        ...
        : t("settings.rules.unrecognisedOption", { value: option }),
    [kind, known, t],
  );

  const options = useMemo(
    () =>
      ruleOptions(known, value).map((option) => ({
        id: option,
        value: option,
  ...
  const label = t(`settings.rules.${kind}Label`);
  ...
  const hint = t(hintKey ?? `settings.rules.${kind}Hint`, { defaultValue: "" }) || undefined;
 ...
 */
function RuleTryIt({ draft, descriptors, resetKey, disabled, testID }: RuleTryItProps) {
  const { t } = useTranslation();
  const [sample, setSample] = useState("");

  const verdict = useMemo(() => {
    const rule = applyRuleDraft({ existing: null, draft, id: "try-it", descriptors });
    return dryRunRuleSample(rule, sample).verdict;
  }, [descriptors, draft, sample]);

  const label = t("settings.rules.tryItLabel");

  return (
    <Field label={label} hint={t(`settings.rules.verdicts.${verdictKey(verdict)}`)} testID={testID}>
      <FormTextInput
        initialValue=""
        value={sample}
        resetKey={resetKey}
        ...
        autoCorrect={false}
        editable={!disabled}
        placeholder={t("settings.rules.tryItPlaceholder")}
...
function verdictKey(verdict: ReturnType<typeof dryRunRuleSample>["verdict"]): string {
  return verdict.fires ? "fires" : verdict.because;
}

interface RuleEventPickerProps {
  value: string;
  error: string | undefined;
  disabled: boolean;
  onChange: (kind: PickerKind, value: string) => void;
 ...
 */
function RuleEventPicker({ value, error, disabled, onChange, testID }: RuleEventPickerProps) {
  const { t } = useTranslation();
  if (RULE_EVENT_OPTIONS.length < 2) {
    return null;
  }
  return (
    <RulePicker
      kind="event"
      known={RULE_EVENT_OPTIONS}
      value={value}
 ...
 */
function RuleHostRow({ host, selected, disabled, onToggle }: RuleHostRowProps) {
  const { t } = useTranslation();
    ...
    <View style={styles.hostRow}>
      <View style={styles.hostRowContent}>
        ...
        {host.unavailableMessageKey ? (
          <Text style={settingsStyles.rowHint} testID={`rule-host-reason-${host.serverId}`}>
      ...
      <Switch
        ...
        accessibilityLabel={t("settings.rules.hostToggle", { host: host.serverName })}
        testID={`rule-host-${host.serverId}`}
      />
    </View>
  );
}

interface RuleOutcomeFieldProps {
  parameter: RuleOutcomeParameter;
  value: string;
  disabled: boolean;
  resetKey: string;
...
function RuleOutcomeRow({
  ...
  const labelFor = useCallback(
    (kind: string) => ruleOutcomeLabel({ kind, event, descriptors, t }),
    [descriptors, event, t],
  );
  ...
  const options = useMemo(
    () =>
      ruleOptions(kinds, outcome.kind).map((kind) => ({
        id: kind,
        value: kind,
        label: labelFor(kind),
      })),
  ...
  const label = t("settings.rules.outcomeLabel");
  ...
  const wordingParameter = useMemo<RuleOutcomeParameter>(
    () => ({
      type: "text",
      id: RULE_WORDING_PARAM,
      label: t("settings.rules.wordingLabel"),
      description: `${t("settings.rules.wordingHint")} ${tokenHelp(
        ruleTokensForWording(trigger),
        t,
      )}`,
      multiline: true,
    }),
  ...
  const promptParameters = useMemo(() => {
    const tokens = tokenHelp(ruleTokensForPrompt(event), t);
    const rewritten: RuleOutcomeParameter[] = [];
    ...
    for (const parameter of rewritten) {
      parameter.label = ruleOutcomeParamLabel(outcome.kind, parameter, t);
    }
    return rewritten;
    ...
    <View style={styles.outcomeRow}>
      <View style={styles.outcomePicker}>
        ...
        <Button
          ...
          accessibilityLabel={t("settings.rules.removeOutcome")}
      ...
      {isPlainOutcomeKind(outcome.kind) ? (
        <RuleOutcomeField
          ...
          value={outcome.params[RULE_WORDING_PARAM] ?? ""}
          disabled={disabled}
          resetKey={`${resetKey}-${outcome.kind}`}
          onChange={handleParamChange}
          testID={`${testID}-${RULE_WORDING_PARAM}`}
        />
      ) : null}
...
interface RuleOutcomeListProps {
  draft: RuleDraft;
  descriptors: readonly RuleOutcomeDescriptor[];
  error: string | undefined;
  disabled: boolean;
...
function RuleOutcomeList({
  ...
  onAdd,
  testID,
}: RuleOutcomeListProps) {
  const { t } = useTranslation();
  const canAdd = nextRuleOutcomeKind(draft, descriptors) !== null;

  return (
    <Field
      label={t("settings.rules.outcomesLabel")}
      hint={t("settings.rules.outcomesHint")}
      error={error}
      testID={testID}
    >
      <View style={styles.outcomeList}>
        {draft.outcomes.map((outcome, index) => (
          <RuleOutcomeRow
            ...
            outcome={outcome}
            kinds={ruleOutcomeKindsForRow(draft, index, descriptors)}
        ...
        {canAdd ? (
          <Button
            ...
            onPress={onAdd}
            disabled={disabled}
            accessibilityLabel={t("settings.rules.addOutcome")}
            testID={`${testID}-add`}
          >
            {t("settings.rules.addOutcome")}
          </Button>
        ) : null}
      </View>
    </Field>
...
export function RuleEditModal({
  ...
  useEffect(() => {
    ...
    setDraft(withDefaultRuleWording(initialDraftRef.current, t));
  ...
  const handleOutcomeKindChange = useCallback(
    (index: number, kind: string) => {
      setDraft((current) => setRuleOutcomeKind(current, index, kind));
      clearOutcomeError();
    },
    [clearOutcomeError],
  );
  ...
  const handleOutcomeParamChange = useCallback((index: number, id: string, value: string) => {
    setDraft((current) => setRuleOutcomeParam(current, index, id, value));
  }, []);

  const handleOutcomeRemove = useCallback(
    (index: number) => {
      setDraft((current) => removeRuleOutcome(current, index));
      clearOutcomeError();
    },
    [clearOutcomeError],
  );
  ...
  const handleOutcomeAdd = useCallback(() => {
    ...
    setDraft((current) =>
      withDefaultRuleWording(
        addRuleOutcome(current, nextRuleOutcomeKind(current, outcomeDescriptors)),
        t,
      ),
    );
    clearOutcomeError();
  ...
  const setField = useCallback((field: keyof RuleDraft, value: string) => {
    ...
    setFieldErrors((current) => {
      ...
      const next = { ...current };
      delete next[field as RuleField];
      return next;
    });
  }, []);
  ...
  const handlePickerChange = useCallback(
    (kind: PickerKind, value: string) => {
      ...
      setDraft((current) =>
        kind === "event"
          ? applyRuleEventChange(current, value, outcomeDescriptors)
          : { ...current, [kind]: value },
      );
  ...
  const triggerOptions = useMemo(() => ruleTriggerOptions(draft.event), [draft.event]);
  ...
  const handleSave = useCallback(async () => {
    ...
    setSubmitError(null);
    const gate = gateRuleSave({ draft, hostCount: hosts.length, serverIds });
    if (gate.kind === "fieldErrors") {
      setFieldErrors(gate.errors);
      return;
    }
    if (gate.kind === "hostsRequired") {
      setSubmitError(t("settings.rules.hostsRequired"));
      return;
    }
    setIsPending(true);
    try {
  ...
  const seamOutcomes = useMemo(
    () => ruleOutcomeOptions(draft.event, outcomeDescriptors),
  ...
  const thresholdLabel = t(
    isTextRule ? "settings.rules.textLabel" : "settings.rules.thresholdLabel",
  );
  const thresholdHint = isTextRule ? t("settings.rules.textHint") : undefined;
    ...
    >
      <View style={styles.body}>
        ...
        <RulePicker
          ...
          hintKey={isAlwaysTrigger ? "settings.rules.triggerHintAlways" : undefined}
          value={draft.trigger}
        ...
        {ruleChoosesHosts(hosts.length) ? (
          <Field
            label={t("settings.rules.hostsLabel")}
            hint={t("settings.rules.hostsHint")}
            testID={`${prefix}-hosts`}
          >
        ...
        {submitError ? (
          <Text style={styles.submitError} testID="rule-submit-error">
            {submitError}
          </Text>
        ) : null}
        ...
        <View style={styles.footer}>
          ...
          <Button
            ...
            testID="rule-save"
          >
            {isPending ? t("settings.rules.saving") : t("settings.rules.save")}
          </Button>
        </View>
      </View>
    </AdaptiveModalSheet>
