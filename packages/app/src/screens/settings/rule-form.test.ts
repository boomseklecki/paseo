...
import {
  ...
  ruleChoosesHosts,
  ruleExampleToDraft,
  applyRuleEventChange,
  addRuleOutcome,
  nextRuleOutcomeKind,
  ruleOutcomeKindOptions,
  ruleOutcomeKindsForRow,
  ruleOutcomeOptions,
  removeRuleOutcome,
  setRuleOutcomeKind,
  setRuleOutcomeParam,
...
function draft(overrides: Partial<RuleDraft> = {}): RuleDraft {
  return { ...toRuleDraft(RULE), ...overrides };
}
...
describe("toRuleDraft", () => {
  it("renders the threshold as a string and carries no rule-level message", () => {
    expect(toRuleDraft(RULE)).toEqual({
...
describe("applyRuleDraft", () => {
  it("parses the threshold back to a number", () => {
    const saved = applyRuleDraft({
      existing: RULE,
  ...
  it("keeps a field it does not recognise", () => {
    const saved = applyRuleDraft({
      existing: { ...RULE, severity: "high" } as Rule,
      draft: draft({ value: "60" }),
      id: RULE.id,
    });
  ...
  it("builds a rule with no existing record, written in both vocabularies", () => {
    const saved = applyRuleDraft({ existing: null, draft: draft(), id: "new-one" });
...
describe("validateRuleDraft", () => {
  ...
  it("rejects a threshold that is empty or not a number", () => {
    expect(validateRuleDraft(draft({ value: "" })).value).toBeDefined();
    expect(validateRuleDraft(draft({ value: "soon" })).value).toBeDefined();
  });

  it("accepts a negative or fractional threshold", () => {
    expect(validateRuleDraft(draft({ value: "0.5" }))).toEqual({});
    expect(validateRuleDraft(draft({ value: "-1" }))).toEqual({});
  });

  it("rejects an empty picker value", () => {
    expect(validateRuleDraft(draft({ trigger: "" })).trigger).toBeDefined();
    expect(validateRuleDraft(draft({ outcomes: [] })).outcomes).toBeDefined();
  });
  ...
  it("rejects an outcome listed twice", () => {
    ...
    ).toBe("settings.rules.outcomeDuplicate");
  });
...
describe("ruleOptions", () => {
  it("offers the known values unchanged when the current one is among them", () => {
    expect(ruleOptions(RULE_OPERATOR_OPTIONS, "gte")).toEqual([...RULE_OPERATOR_OPTIONS]);
  });
  ...
  it("adds an unrecognised current value so it stays selectable", () => {
    expect(ruleOptions(RULE_OPERATOR_OPTIONS, "approaches")).toContain("approaches");
  });

  it("does not add an empty current value", () => {
    expect(ruleOptions(RULE_OPERATOR_OPTIONS, "")).toEqual([...RULE_OPERATOR_OPTIONS]);
  });
});
...
describe("moveRule", () => {
  ...
  it("moves a rule up one place", () => {
    expect(moveRule(rules, "c", "up")).toEqual(["a", "c", "b"]);
  });

  it("moves a rule down one place", () => {
    expect(moveRule(rules, "a", "down")).toEqual(["b", "a", "c"]);
  });
  ...
  it("leaves the order alone at either end", () => {
    expect(moveRule(rules, "a", "up")).toEqual(["a", "b", "c"]);
    expect(moveRule(rules, "c", "down")).toEqual(["a", "b", "c"]);
  });

  it("leaves the order alone for a rule that is not there", () => {
    expect(moveRule(rules, "missing", "up")).toEqual(["a", "b", "c"]);
  });
});

describe("applyRuleDraft ordering", () => {
...
describe("previewRuleMessage", () => {
  ...
  it("has nothing to preview for a rule with no message", () => {
    expect(previewRuleMessage(RULE, t)).toBeNull();
  });
});

describe("describeRule", () => {
  it("reads as a sentence in the same units the toast uses", () => {
    expect(describeRule(RULE, t)).toBe("settings.rules.triggers.idleSeconds ≥ 1h");
  });

  it("formats a percentage and a cost", () => {
    expect(describeRule({ ...RULE, trigger: "agent.contextUsedPercent", value: 80 }, t)).toContain(
      "80%",
    );
    expect(describeRule({ ...RULE, trigger: "agent.sessionCostUsd", value: 10 }, t)).toContain(
      "$10.00",
    );
  });
...
describe("ruleChoosesHosts", () => {
  it("offers no choice when there is nowhere else for a rule to go", () => {
    expect(ruleChoosesHosts(0)).toBe(false);
    expect(ruleChoosesHosts(1)).toBe(false);
    expect(ruleChoosesHosts(2)).toBe(true);
  });
});

describe("gateRuleSave", () => {
  it("saves a complete draft with a host chosen", () => {
    expect(gateRuleSave({ draft: draft(), hostCount: 2, serverIds: ["a"] })).toEqual({
      kind: "save",
    });
  });

  it("reports field errors and says which fields", () => {
    const gate = gateRuleSave({
  ...
  it("holds the host complaint back until the fields pass", () => {
    expect(
      gateRuleSave({ draft: draft({ value: "" }), hostCount: 2, serverIds: [] }),
  ...
  it("refuses a save that would leave the rule on no host", () => {
    expect(gateRuleSave({ draft: draft(), hostCount: 2, serverIds: [] })).toEqual({
      kind: "hostsRequired",
    });
  });
  ...
  it("does not ask for a host the editor never offered", () => {
    expect(gateRuleSave({ draft: draft(), hostCount: 1, serverIds: [] })).toEqual({
      kind: "save",
    });
  });
});

describe("ruleExampleToDraft", () => {
  it("opens a redirect example with its outcome and parameters filled in", () => {
    const built = ruleExampleToDraft({
      id: "aside-on-btw",
...
describe("describeRule for a text rule", () => {
  ...
  it("names the message trigger and spells the operator as a phrase", () => {
    ...
    expect(described).toBe(
      "settings.rules.triggers.message settings.rules.operatorPhrases.startsWith /btw",
    );
  });
});
...
describe("what a seam offers the editor", () => {
  it("offers the message trigger only where a message is being sent", () => {
    expect(ruleTriggerOptions("message.send")).toContain("message");
    expect(ruleTriggerOptions("turn.failed")).not.toContain("message");
  });
  ...
  it("offers the composer's set for a seam it has never heard of", () => {
    expect(ruleOutcomeKindOptions("moon.rose", ASIDE_DESCRIPTORS)).toEqual(["warn", "block"]);
    expect(ruleTriggerOptions("moon.rose").length).toBeGreaterThan(0);
  });
  ...
  it("offers only the outcomes a seam will carry out", () => {
    ...
    expect(ruleOutcomeOptions("message.send", descriptors).map((d) => d.kind)).toEqual(["aside"]);
  });
});

describe("applyRuleEventChange", () => {
  ...
  it("drops a trigger the new seam cannot use", () => {
    const moved = applyRuleEventChange(draft({ trigger: "message" }), "turn.failed");

    expect(moved.event).toBe("turn.failed");
    expect(moved.trigger).not.toBe("message");
    expect(ruleTriggerOptions("turn.failed")).toContain(moved.trigger);
  });
  ...
  it("drops an outcome the new seam cannot carry out", () => {
    const moved = applyRuleEventChange(
...
describe("ruleOutcomeKindOptions", () => {
  ...
  it("offers the plain kinds and every runnable one the daemon described", () => {
    expect(ruleOutcomeKindOptions("message.send", ASIDE_DESCRIPTORS)).toEqual([
      "warn",
      "block",
      "aside",
    ]);
    expect(ruleOutcomeKindOptions("message.send", [])).not.toContain("redirect");
  });
  ...
  it("leaves out a runnable kind the daemon did not describe", () => {
    expect(ruleOutcomeKindOptions("message.send", [])).toEqual(["warn", "block"]);
  });

  it("offers only what a daemon seam accepts", () => {
    const kinds = ruleOutcomeKindOptions("turn.failed", ASIDE_DESCRIPTORS);
...
describe("describeRuleOutcome", () => {
  ...
  it("translates a plain kind", () => {
    expect(describeRuleOutcome(RULE, t2).label).toBe("Block");
    expect(describeRuleOutcome(RULE, t2).isBlocking).toBe(true);
  });
  ...
  it("names a known kind in the app's own words, not the daemon's", () => {
    ...
    expect(describeRuleOutcome(aside, t2, ASIDE_DESCRIPTORS).label).toBe(
      "settings.rules.outcomeKinds.asideAtSend",
    );
  });
  ...
  it("falls back to the daemon's label for a kind it does not know", () => {
    ...
    expect(
      describeRuleOutcome(teleport, t2, [{ kind: "teleport", label: "Teleport", parameters: [] }])
        .label,
    ).toBe("Teleport");
  });
  ...
  it("falls back to the raw kind when nothing described it either", () => {
    ...
    expect(describeRuleOutcome(teleport, t2, []).label).toBe("teleport");
  });
  ...
  it("names the most severe outcome and counts the others", () => {
    ...
    expect(describeRuleOutcome(both, t2, ASIDE_DESCRIPTORS).label).toBe(
      "settings.rules.outcomeKinds.asideAtSend +1",
    );
  });
  ...
  it("names an aside differently at a seam where nobody typed anything", () => {
    ...
    expect(describeRuleOutcome(atFailure, t2, ASIDE_DESCRIPTORS).label).toBe("Ask on the side");
    expect(describeRuleOutcome(atSend, t2, ASIDE_DESCRIPTORS).label).toBe(
      "settings.rules.outcomeKinds.asideAtSend",
    );
  });
});

describe("ruleOutcomeKindsForRow", () => {
  ...
  it("leaves out what a sibling row already holds", () => {
    ...
    expect(ruleOutcomeKindsForRow(two, 0, ASIDE_DESCRIPTORS)).toEqual(["warn", "aside"]);
    expect(ruleOutcomeKindsForRow(two, 1, ASIDE_DESCRIPTORS)).toEqual(["block", "aside"]);
  });

  it("always keeps the row's own kind, or the picker would show nothing", () => {
    const one = draft({ outcomes: [{ kind: "warn", params: {} }] });

    expect(ruleOutcomeKindsForRow(one, 0, [])).toContain("warn");
  });
});
...
describe("editing the outcome list", () => {
  it("adds the first kind the seam accepts that is not already listed", () => {
    ...
    expect(nextRuleOutcomeKind(one, ASIDE_DESCRIPTORS)).toBe("block");
    expect(addRuleOutcome(one, "block").outcomes).toEqual([
      { kind: "warn", params: {} },
      { kind: "block", params: {} },
    ]);
  });
  ...
  it("has nothing left to add once every kind is listed", () => {
    ...
    expect(nextRuleOutcomeKind(full, [])).toBeNull();
    expect(addRuleOutcome(full, null)).toBe(full);
  });
  ...
  it("refuses to empty the list", () => {
    ...
    expect(removeRuleOutcome(one, 0)).toBe(one);
  });
  ...
  it("removes by position", () => {
    ...
    expect(removeRuleOutcome(two, 0).outcomes).toEqual([{ kind: "block", params: {} }]);
  });
  ...
  it("keeps what was typed when the kind changes", () => {
    ...
    expect(setRuleOutcomeKind(one, 0, "start").outcomes).toEqual([
  ...
  it("sets one parameter on one row", () => {
    ...
    expect(setRuleOutcomeParam(two, 1, "title", "Continued").outcomes).toEqual([
      { kind: "aside", params: {} },
...
describe("wording lives on the outcome", () => {
  ...
  it("carries an old rule's single message onto the outcome that displayed it", () => {
    const built = toRuleDraft({ ...RULE, message: "Too cold." });

    expect(built.outcomes).toEqual([{ kind: "block", params: { wording: "Too cold." } }]);
  });
});

describe("withDefaultRuleWording", () => {
  ...
  it("fills a plain outcome with the sentence that would fire", () => {
    const filled = withDefaultRuleWording(draft({ outcomes: [{ kind: "block", params: {} }] }), t);

    expect(filled.outcomes[0]?.params.wording).toBe("composer.rules.idleSeconds");
  });

  it("leaves a sentence someone wrote alone", () => {
    const filled = withDefaultRuleWording(
  ...
  it("adds nothing to an outcome that shows no wording", () => {
    const filled = withDefaultRuleWording(draft({ outcomes: [{ kind: "aside", params: {} }] }), t);
