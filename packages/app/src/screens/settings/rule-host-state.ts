...
export function resolveHostUnavailableMessageKey(input: {
  ...
  if (!input.isConnected) {
    return "settings.rules.unavailableDisconnected";
  }
  if (!input.isSupported) {
    return "settings.rules.unavailableUnsupported";
  }
  return null;
}
 ...
 */
export function resolveRulesListState(input: {
  isConnected: boolean;
  isSupported: boolean;
  hasFailed: boolean;
...
export function listStateMessageKey(state: RulesListState): string {
  switch (state.kind) {
    case "unavailable":
      return state.messageKey;
    case "failed":
      return "settings.rules.loadFailed";
    case "loading":
      return "settings.rules.loading";
    default:
      return "settings.rules.emptyState";
  }
}
