 ...
 */
export function explainPreSendCheckStructure(
  rule: PreSendCheckRule,
): Exclude<PreSendDryRunVerdict, { fires: true }> | null {
  const normalized = normalizePreSendCheckRule(rule);
  if (!normalized.enabled) {
    return { fires: false, because: "disabled" };
  }
  const definition = findPreSendEventDefinition(normalized.event);
  if (!definition) {
 ...
 */
export function preSendCheckNeverFires(rule: PreSendCheckRule): boolean {
  const structural = explainPreSendCheckStructure(rule);
 ...
 */
export function dryRunPreSendCheckSample(
  rule: PreSendCheckRule,
  sample: string,
): PreSendDryRunResult {
  const { trigger } = normalizePreSendCheckRule(rule);
