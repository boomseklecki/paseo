...
import { evaluateRules, firstRunnableRuleOutcome } from "./evaluate.js";
import { isTextTrigger, RULE_TEXT_TRIGGERS, RULE_TRIGGER_UNITS, RULE_TRIGGERS } from "./types.js";
  ...
  for (const { operator, below, at, above } of cases) {
    it(`${operator} fires below/at/above threshold as ${below}/${at}/${above}`, () => {
      ...
      const fired = (idleSeconds: number) =>
        evaluateRules(rules, context({ "agent.idleSeconds": idleSeconds })).disposition === "block";
...
describe("evaluateRules measurements", () => {
  it("reads contextUsedPercent", () => {
    const rules = [rule({ measurement: "agent.contextUsedPercent", threshold: 80 })];
    expect(evaluateRules(rules, context({ "agent.contextUsedPercent": 81 })).disposition).toBe(
      "block",
    );
    expect(evaluateRules(rules, context({ "agent.contextUsedPercent": 79 })).disposition).toBe(
      "allow",
    );
  });
  ...
  it("reads sessionCostUsd", () => {
    ...
    expect(evaluateRules(rules, context({ "agent.sessionCostUsd": 5.01 })).disposition).toBe(
      "block",
    );
    expect(evaluateRules(rules, context({ "agent.sessionCostUsd": 4.99 })).disposition).toBe(
      "allow",
    );
  });
});

describe("evaluateRules findings", () => {
  ...
  it("reports a null message when the rule carries none", () => {
    const evaluation = evaluateRules([rule()], context({ "agent.idleSeconds": 250 }));
...
describe("evaluateRules aggregation", () => {
  it("allows with no findings when nothing trips", () => {
    const evaluation = evaluateRules([rule()], context({ "agent.idleSeconds": 1 }));
...
// understand.
describe("evaluateRules fails open", () => {
  const tripping = context({ "agent.idleSeconds": 250 });

  const unreadable: Array<[string, Rule, RuleMeasurementContext]> = [
  ...
  for (const [name, unreadableRule, measurements] of unreadable) {
    it(`skips a rule with an ${name}`, () => {
      expect(evaluateRules([unreadableRule], measurements)).toEqual({
        disposition: "allow",
        findings: [],
      });
    });
  ...
  it("allows an empty rule list", () => {
    expect(evaluateRules([], tripping)).toEqual({
      disposition: "allow",
      findings: [],
    });
  });
...
function trigger(overrides: Partial<Rule> = {}): Rule {
  return {
    id: "aside",
    measurement: "message",
    operator: "startsWith",
...
describe("text triggers", () => {
  it("fires on a message that starts with the trigger", () => {
    const evaluation = evaluateRules([trigger()], context({ message: "/btw what is this" }));
  ...
  it("ignores leading whitespace and case", () => {
    expect(evaluateRules([trigger()], context({ message: "  /BTW quick one" })).disposition).toBe(
      "redirect",
    );
  });
  ...
  it("does not fire when the trigger appears mid-message", () => {
    expect(
      evaluateRules([trigger()], context({ message: "explain how /btw works" })).disposition,
    ).toBe("allow");
  });

  it("matches anywhere for contains", () => {
    const rules = [trigger({ operator: "contains" })];
    expect(evaluateRules(rules, context({ message: "explain how /btw works" })).disposition).toBe(
      "redirect",
    );
  });
  ...
  it("declines a redirect whose action kind is unknown", () => {
    ...
    expect(evaluateRules(rules, context({ message: "/btw hi" })).disposition).toBe("allow");
  });
  ...
  it("declines a trigger with empty text rather than matching everything", () => {
    ...
    expect(evaluateRules(rules, context({ message: "anything" })).disposition).toBe("allow");
  });
...
describe("a rule with several outcomes", () => {
  ...
  it("keeps every outcome on the finding", () => {
    const evaluation = evaluateRules([both], context({ message: "/btw quick one" }));
  ...
  it("takes its disposition from the most severe outcome", () => {
    expect(evaluateRules([both], context({ message: "/btw" })).disposition).toBe("redirect");
  });
...
describe("firstRunnableRuleOutcome", () => {
  ...
  it("takes the first runnable outcome across the findings in order", () => {
    ...
    expect(firstRunnableRuleOutcome(evaluation.findings)).toEqual({ kind: "aside" });
  });
  ...
  it("answers null when nothing that tripped asked for a runner", () => {
    ...
    expect(firstRunnableRuleOutcome(evaluation.findings)).toBeNull();
  });
});
 ...
 *
 * Before it, `RuleTrigger` was declared and referenced nowhere: a trigger
 * added to `RULE_TRIGGERS` compiled clean, was offered by the editor, saved
...
describe("every declared trigger is wired end to end", () => {
  it("gives each trigger a unit", () => {
    for (const declared of RULE_TRIGGERS) {
      expect(RULE_TRIGGER_UNITS[declared]).toBeTypeOf("string");
    }
  });

  // The list that used to be written out twice.
  it("derives the text triggers from the unit table", () => {
    expect([...RULE_TEXT_TRIGGERS]).toEqual([
      "agent.lastError",
      "agent.provider",
      "agent.model",
      "message",
  ...
  it("reads each numeric trigger from its own key", () => {
    for (const declared of RULE_TRIGGERS) {
      if (RULE_TRIGGER_UNITS[declared] === "text" || declared === "always") {
        continue;
      }
      const numeric = rule({
        trigger: declared,
      ...
      expect(evaluateRules([numeric], context({ [declared]: 11 })).disposition).toBe("block");
      expect(evaluateRules([numeric], context({ [declared]: 9 })).disposition).toBe("allow");
      expect(evaluateRules([numeric], context()).disposition).toBe("allow");
    }
  });
  ...
  it("skips a trigger this build never declared", () => {
    ...
    expect(evaluateRules([unknown], context({ "agent.phaseOfMoon": 500 })).findings).toEqual([]);
  });
  ...
  it("does not read a string as a number", () => {
    ...
    expect(evaluateRules([crossed], context({ message: "hello" })).findings).toEqual([]);
  });
});
