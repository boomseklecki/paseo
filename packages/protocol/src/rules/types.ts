 ...
 *
 * Rules are stored one per file under `<PASEO_HOME>/rules/`, read from
 ...
 */
export const RULE_ID_PATTERN = /^[A-Za-z0-9._-]{1,120}$/;
...
export const RuleSchema = z
  .object({
    ...
    /**
     * COMPAT(ruleVocabulary): added in v0.3.2, remove after 2027-02-10
     ...
     */
    outcomes: z.array(RuleOutcomeSchema).optional(),
    /**
     * COMPAT(ruleOutcomeList): added in v0.3.2, remove after 2027-02-10
     ...
     */
    outcome: RuleOutcomeSchema.optional(),
     ...
     */
    action: RuleOutcomeSchema.optional(),
    /**
 ...
 */
export const RULE_ALWAYS_TRIGGER = "always";
 ...
 * `RULE_TRIGGERS` without an entry here is a type error rather than a rule
 * that silently never fires. That is the guarantee `COMPARATORS` has given the
 * operators from the start, and the triggers went without it: `RuleTrigger`
 ...
 */
export type RuleTriggerUnit = "seconds" | "percent" | "usd" | "tokens" | "text" | "none";

export const RULE_TRIGGER_UNITS: Record<RuleTrigger, RuleTriggerUnit> = {
 ...
 */
export const RULE_TRIGGER_UNITS_BY_NAME = RULE_TRIGGER_UNITS as Record<
  string,
  RuleTriggerUnit | undefined
>;
 ...
 */
export const RULE_TEXT_TRIGGERS = RULE_TRIGGERS.filter(
  (trigger) => RULE_TRIGGER_UNITS[trigger] === "text",
);

export function isTextTrigger(trigger: string): boolean {
  return RULE_TRIGGER_UNITS_BY_NAME[trigger] === "text";
}
 ...
 */
export const RULE_NUMERIC_OPERATORS = ["gt", "gte", "lt", "lte"] as const;
 ...
 * not get redirected.
 */
export const RULE_TEXT_OPERATORS = ["startsWith", "contains"] as const;

export const RULE_OPERATORS = [...RULE_NUMERIC_OPERATORS, ...RULE_TEXT_OPERATORS] as const;

export type RuleNumericOperator = (typeof RULE_NUMERIC_OPERATORS)[number];
 ...
 */
export const RULE_RUNNABLE_OUTCOME_KINDS = ["aside", "fork", "start", "schedule"] as const;

export type RuleRunnableOutcomeKind = (typeof RULE_RUNNABLE_OUTCOME_KINDS)[number];

export type RuleOutcome = z.infer<typeof RuleOutcomeSchema>;

/** One outcome as a runner is asked to carry it out. Structurally a `RuleOutcome`. */
export type RuleOutcomeSpec = RuleOutcome;

export function isRuleRunnableOutcomeKind(kind: string): boolean {
  return (RULE_RUNNABLE_OUTCOME_KINDS as readonly string[]).includes(kind);
}
 ...
 */
export const RULE_PLAIN_OUTCOME_KINDS = ["warn", "block", "notify"] as const;

export type RulePlainOutcomeKind = (typeof RULE_PLAIN_OUTCOME_KINDS)[number];

/** What an editor offers: the plain outcomes plus every runnable one this build has. */
export const RULE_OUTCOME_KINDS = [
  ...RULE_PLAIN_OUTCOME_KINDS,
  ...RULE_RUNNABLE_OUTCOME_KINDS,
] as const;

export function isPlainOutcomeKind(kind: string): boolean {
  return (RULE_PLAIN_OUTCOME_KINDS as readonly string[]).includes(kind);
}
 ...
 */
export const RULE_VALUE_TOKEN = "{{value}}";
export const RULE_THRESHOLD_TOKEN = "{{threshold}}";
export const RULE_DURATION_TOKEN = "{{duration}}";
export const RULE_MESSAGE_TOKEN = "{{message}}";
 ...
 */
export function ruleTokensForPrompt(event: string): readonly string[] {
  return event === "message.send" ? [RULE_MESSAGE_TOKEN, RULE_VALUE_TOKEN] : [RULE_VALUE_TOKEN];
}
 ...
 */
export function ruleTokensForWording(trigger: string): readonly string[] {
  const tokens = [RULE_VALUE_TOKEN, RULE_THRESHOLD_TOKEN];
  return trigger === "agent.idleSeconds" ? [...tokens, RULE_DURATION_TOKEN] : tokens;
}

/** What one outcome says, or `undefined` to fall back to a translated default. */
export function ruleOutcomeWording(outcome: RuleOutcome): string | undefined {
 ...
 */
export function ruleOutcomeSeverity(kind: string): number {
  if (kind === "warn") {
    return 1;
  }
  if (kind === "block") {
  ...
  return isRuleRunnableOutcomeKind(kind) || kind === "notify" ? 3 : 0;
}
 ...
 */
export function mostSevereRuleOutcome(outcomes: readonly RuleOutcome[]): RuleOutcome {
  const [first] = outcomes;
  if (!first) {
    throw new Error("A rule needs at least one outcome");
  }
  return outcomes.reduce(
    (worst, candidate) =>
      ruleOutcomeSeverity(candidate.kind) > ruleOutcomeSeverity(worst.kind) ? candidate : worst,
    first,
  );
}
 ...
 */
export type RuleMeasurementContext = Partial<Record<RuleTrigger, number | string | null>>;

export type RuleDisposition = "allow" | "warn" | "block" | "redirect";
 ...
 * heard of.
 */
export const RuleOutcomeParameterSchema = z.discriminatedUnion("type", [
  z.object({
    type: z.literal("text"),
    id: z.string(),
    label: z.string(),
...
export type RuleOutcomeParameter = z.infer<typeof RuleOutcomeParameterSchema>;

/** What one outcome is and what it takes, as the daemon describes itself. */
export const RuleOutcomeDescriptorSchema = z.object({
  kind: z.string(),
  label: z.string(),
  description: z.string().optional(),
  parameters: z.array(RuleOutcomeParameterSchema),
});

export type RuleOutcomeDescriptor = z.infer<typeof RuleOutcomeDescriptorSchema>;
...
export const RuleExampleSchema = z.object({
  ...
  rule: z
    .object({
      ...
      operator: z.string(),
      value: z.union([z.string(), z.number()]).optional(),
      outcomes: z.array(RuleOutcomeSchema).min(1),
      message: z.string().optional(),
    })
    .passthrough(),
});
