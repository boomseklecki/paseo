...
describe("normalizeRule", () => {
  it("reads a numeric rule written in the old vocabulary", () => {
    expect(normalizeRule(OLD_NUMERIC)).toEqual({
      id: "cold-prompt-cache",
  ...
  it("resolves an old redirect to the kind its action named", () => {
    const normalized = normalizeRule(OLD_REDIRECT);
  ...
  it("leaves an old redirect that named no action unperformable", () => {
    ...
    expect(normalizeRule(withoutAction).outcomes).toEqual([{ kind: "redirect" }]);
  });
  ...
  it("falls through an empty outcome list to the older fields", () => {
    expect(normalizeRule({ ...OLD_NUMERIC, outcomes: [] }).outcomes).toEqual([{ kind: "block" }]);
  });

  it("defaults the event, since every rule predating it meant the send", () => {
    expect(normalizeRule(OLD_NUMERIC).event).toBe("message.send");
    expect(normalizeRule({ ...OLD_NUMERIC, event: "turn.failed" }).event).toBe("turn.failed");
  });

  // Absent means on, so a hand-written rule needs no boilerplate to be live.
  it("treats only an explicit false as off", () => {
    expect(normalizeRule(OLD_NUMERIC).enabled).toBe(true);
    expect(normalizeRule({ ...OLD_NUMERIC, enabled: false }).enabled).toBe(false);
    expect(normalizeRule({ ...OLD_NUMERIC, enabled: true }).enabled).toBe(true);
  });
});

describe("projectRule", () => {
  const numeric: NormalizedRule = {
    id: "cold-prompt-cache",
    event: DEFAULT_RULE_EVENT,
    trigger: "agent.idleSeconds",
    operator: "gte",
  ...
  it("writes both vocabularies for a numeric rule", () => {
    expect(projectRule(numeric)).toEqual({
      id: "cold-prompt-cache",
      event: "message.send",
      trigger: "agent.idleSeconds",
      measurement: "agent.idleSeconds",
  ...
  it("writes a text operand to text and a numeric one to threshold", () => {
    const projected = projectRule({ ...numeric, trigger: "message", value: "/btw" });
  ...
  it("refuses a rule with no outcomes rather than inventing one", () => {
    expect(() => projectRule({ ...numeric, outcomes: [] })).toThrow(/at least one outcome/);
  });

  it("writes no action for a plain outcome", () => {
    expect(projectRule({ ...numeric, outcomes: [{ kind: "warn" }] })).not.toHaveProperty("action");
  });
  ...
  it("writes enabled only when the rule is off", () => {
    expect(projectRule(numeric)).not.toHaveProperty("enabled");
    expect(projectRule({ ...numeric, enabled: false }).enabled).toBe(false);
  });

  it("omits an absent message and order rather than writing undefined", () => {
    const projected = projectRule(numeric);
  ...
  it("produces something the rule schema accepts", () => {
    for (const outcomes of [[{ kind: "warn" }], [{ kind: "block" }], [{ kind: "aside" }]]) {
      expect(() => RuleSchema.parse(projectRule({ ...numeric, outcomes }))).not.toThrow();
    }
  });
});
...
describe("the two together", () => {
  ...
  it("round-trips a rule written in the old vocabulary", () => {
    for (const rule of [OLD_NUMERIC, OLD_REDIRECT]) {
      const once = normalizeRule(rule);
      const twice = normalizeRule(projectRule(once));
  ...
  it("round-trips a rule that only ever had the new names", () => {
    ...
    expect(normalizeRule(projectRule(normalizeRule(newOnly)))).toEqual(normalizeRule(newOnly));
  });
});
