...
export class AsideOutcome {
  ...
  constructor(options: AsideOutcomeOptions) {
    ...
    this.logger = options.logger.child({ module: "rules", outcome: "aside" });
 ...
 */
function buildQuestion(request: RuleOutcomeRequest): string {
  const template = request.outcome.prompt?.trim();
  const message = request.message.trim();
  if (!template) {
    return message;
  }
  if (promptUsesRuleToken(template)) {
    return renderRulePrompt(template, request);
  }
