...
import { evaluateRuleEvent } from "@getpaseo/protocol/rules/evaluate";
...
export class RuleEventTracker {
   ...
   * not a pure function.
   */
  fired(agentId: string, event: string, findings: readonly RuleEventFinding[]): RuleEventFinding[] {
    const key = RuleEventTracker.key(agentId, event);
 ...
 *
 * The two halves are separable and tested apart: `evaluateRuleEvent` is pure
 * and says what is true, the tracker says what is news.
 */
export function fireRuleEvent(
  tracker: RuleEventTracker,
  input: RuleEventInput,
): RuleEventFinding[] {
  const findings = evaluateRuleEvent(input.rules, input.event, input.context);
