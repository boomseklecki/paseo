...
beforeEach(async () => {
  paseoHome = await mkdtemp(join(tmpdir(), "rules-service-test-"));
  dir = join(paseoHome, "rules");
  service = new RulesService({ paseoHome, logger: createTestLogger() });
});
...
async function writeRule(id: string, overrides: Partial<Rule> = {}): Promise<void> {
  await writeFile(
    join(dir, `${id}.json`),
    JSON.stringify({
      ...projectRule({
