...
import { RulesSession, type RuleOutcomeRunner } from "./rules-session.js";
import { createStub } from "../../test-utils/class-mocks.js";
import { findByType } from "../../test-utils/session-stubs.js";
import type { SessionOutboundMessage } from "../../messages.js";
import type { RuleOutcomeResult } from "../../rules/outcomes/types.js";
...
function runner(run: RuleOutcomeRunner["run"]): RuleOutcomeRunner {
  return { run };
}
...
function makeSession(
  service: { [K in keyof RulesService]?: unknown },
  outcomeRunner: RuleOutcomeRunner = DECLINE_EVERYTHING,
) {
  const emitted: SessionOutboundMessage[] = [];
  const session = new RulesSession({
    host: { emit: (message) => emitted.push(message) },
    rulesService: createStub<RulesService>(service),
    outcomeRunner,
...
describe("RulesSession", () => {
  it("answers a list with the rules and the outcomes this daemon can run", async () => {
    ...
    await session.handleRulesListRequest({
  ...
  it("answers each write with the whole resulting list", async () => {
    ...
    await session.handleRulesUpsertRequest({
      type: "rules.upsert.request",
      requestId: "r1",
      check: RULE,
    });
    await session.handleRulesDeleteRequest({
      type: "rules.delete.request",
      requestId: "r2",
      ruleId: RULE.id,
    });
    await session.handleRulesReorderRequest({
  ...
  it("passes a run_outcome through to the runner and reports what it started", async () => {
    ...
    await session.handleRulesRunOutcomeRequest({
  ...
  it("does not let a declined outcome carry a subagent id", async () => {
    const declined: RuleOutcomeResult = { status: "declined", reason: "No such agent" };
    const { session, emitted } = makeSession(
      {},
